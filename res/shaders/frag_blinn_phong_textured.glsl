...
void main() {
	...
	vec4 textureColor2 = texture(s_Diffuse2, inUV);
		//Set to no texture
	if(u_textureColorOn==0){
		textureColor1=vec4(1,1,1,1);
		textureColor2=vec4(1,1,1,1);
	}
	...
	vec3 result = (
		...
		((ambient*u_ambientOn) + (diffuse*u_diffuseOn) + (specular*u_specularOn)) * attenuation // light factors from our single light
