...
int main() { 
	...
	// Push another scope so most memory should be freed *before* we exit the app
	{
		...
		shader->SetUniform("u_ambientOn", 1);
		shader->SetUniform("u_diffuseOn", 1);
		shader->SetUniform("u_specularOn", 1);
		shader->SetUniform("u_textureColorOn", textureColorOn);
		...
		BackendHandler::imGuiCallbacks.push_back([&]() {
			if (ImGui::CollapsingHeader("Effect controls"))
			{
				...
				if (activeEffect == 3)
				{
					...
					if (ImGui::SliderFloat("Intensity", &threshold, 0.0f, 1.0f))
					{
			...
			if (ImGui::CollapsingHeader("Debug")) {
				...
				if (debugMode == 1) {
					...
					shader->SetUniform("u_ambientOn", 0);
					shader->SetUniform("u_diffuseOn", 0);
					shader->SetUniform("u_specularOn", 0);
					BGM.SetParameter("Loop 1", 1);
				...
				if (debugMode == 5) { 
					ImGui::Text("Ambient + specular + Bloom");
					BGM.SetParameter("Loop 1", 0);
					BGM.SetParameter("Loop 2", 0);
				...
				if (debugMode == 6) {
					...
					BGM.SetParameter("Loop 2", 0);
					BGM.SetParameter("Loop 3", 1);
					ImGui::Text("Toggle Textures on / off");  
					ImGui::SliderInt("on / off", &textureColorOn, 0, 1);
					shader->SetUniform("u_textureColorOn", textureColorOn);
				}
			}
		...
		Texture2D::sptr kichaup = Texture2D::LoadFromFile("images/kichaup.png");
		Texture2D::sptr wine = Texture2D::LoadFromFile("images/wine_UV.png");
		Texture2D::sptr knife = Texture2D::LoadFromFile("images/knife UV.png");
		...
		ShaderMaterial::sptr kichaupMat = ShaderMaterial::Create();
		kichaupMat->Shader = shader;
		kichaupMat->Set("s_Diffuse", kichaup);
		kichaupMat->Set("s_Specular", noSpec);
		kichaupMat->Set("u_Shininess", 8.0f);
		kichaupMat->Set("u_TextureMix", 0.0f);

		ShaderMaterial::sptr wineMat = ShaderMaterial::Create();
		wineMat->Shader = shader;
		wineMat->Set("s_Diffuse", wine);
		...
		knifeMat->Shader = shader;
		knifeMat->Set("s_Diffuse", knife);
		knifeMat->Set("s_Specular", noSpec);
		knifeMat->Set("u_Shininess", 20.0f);
		...
		GameObject obj3 = scene->CreateEntity("knife");
		{
			VertexArrayObject::sptr vao = ObjLoader::LoadFromFile("models/knife.obj");
			obj3.emplace<RendererComponent>().SetMesh(vao).SetMaterial(knifeMat);
		...
		EnvironmentGenerator::AddObjectToGeneration("models/kicha.obj", kichaupMat, 30,
			spawnFromHere, spawnToHere, allAvoidAreasFrom, allAvoidAreasTo);
		EnvironmentGenerator::AddObjectToGeneration("models/wine.obj", wineMat, 10,
			spawnFromHere, spawnToHere, allAvoidAreasFrom, allAvoidAreasTo);
